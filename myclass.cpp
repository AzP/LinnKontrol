...
void MyClass::scanForNewDevices()
{
	Herqq::Upnp::HDiscoveryType type = Herqq::Upnp::HDiscoveryType::createDiscoveryTypeForAllResources();
	m_controlPoint->scan(type);
}
...
void MyClass::rootDeviceOnline(Herqq::Upnp::HClientDevice* newDevice)
{
	...
	qDebug() << "New device connected: " << QString(info.friendlyName());
}

void MyClass::rootDeviceOffline(Herqq::Upnp::HClientDevice* rootDevice)
{
	Herqq::Upnp::HDeviceInfo info = rootDevice->info();
	qDebug() << "Device disconnected: " << QString(info.friendlyName());
